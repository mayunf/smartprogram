...
class LogManager implements LoggerInterface
{
    ...
    {
        return new Monolog('EasyBaiDu', $this->prepareHandlers([new StreamHandler(
            \sys_get_temp_dir().'/EasyBaiDu/EasyBaiDu.log', $this->level(['level' => 'debug'])
        )]));
    }
    ...
    protected function createSyslogDriver(array $config)
    {
        return new Monolog($this->parseChannel($config), [
            $this->prepareHandler(new SyslogHandler(
                    'EasyBaiDu', $config['facility'] ?? LOG_USER, $this->level($config))
            ),
        ]);
    }
